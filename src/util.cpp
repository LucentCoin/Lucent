...
const char * const BITCOIN_CONF_FILENAME = "lucent.conf";
const char * const BITCOIN_PID_FILENAME = "lucentd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "lucent" is a composite category enabling all Lucent-related debug output
                if(ptrCategory->count(std::string("lucent"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LucentCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LucentCore
    // Mac: ~/Library/Application Support/LucentCore
    // Unix: ~/.lucentcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LucentCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LucentCore";
#else
    // Unix
    return pathRet / ".lucentcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty lucent.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override lucent.conf
